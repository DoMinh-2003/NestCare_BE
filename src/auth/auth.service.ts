...
export class AuthService {
  ...
  async login(payload: any) {
    ...
    if (!user) {
      throw new UnauthorizedException('Invalid Username or Password'); // Ném lỗi 401 nếu sai thông tin
    }
  ...
  async register(payload: any) {
    ...
    if (existingUser) {
      throw new ConflictException('Username already exists'); // Nếu đã tồn tại thì ném lỗi
    }
